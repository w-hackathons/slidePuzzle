...
function imageResize(direction, size){
  ...
  if(direction == 0){
    targetImage.width = size;  // 横幅を400pxにリサイズ
    targetImage.height = orgHeight * (targetImage.width / orgWidth);
  }else{
    targetImage.height = size;  // 横幅を400pxにリサイズ
    targetImage.Width = orgWidth * (targetImage.height / height);
=======
...
>>>>>>> 0b583f706a92c243404c007723324c9d312bbbcb
  }
}
...
function loadImage() {
  ...
  if (canvas.getContext) {
    ...
    if( document.getElementById('url').value !== ""){
      ...
      img.src ="https://3.bp.blogspot.com/-8tO27OaQmKo/W1vhDEBgLMI/AAAAAAABNtc/WqKk_bLT1k0O2loY5VH5UeNp6ejZ3GBwQCLcBGAs/s800/character_earth_chikyu_internet.png"
    }
...
function puzzlechange(){
  const selectValue = document.getElementById("puzzle").value
  BOARD.X_BLOCKS = selectValue;
  BOARD.Y_BLOCKS = selectValue;
  BOARD.DIRECT_MAX = selectValue +1;
  diff = [-BOARD.X_BLOCKS, -1, 1, BOARD.X_BLOCKS];
  shuffle();
}